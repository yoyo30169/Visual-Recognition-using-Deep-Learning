...
class run:
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for e in range(1, self.epochs+1):
                ...
                print("Epoch nr: [{}/{}]  - Loss: {:.5f} - val PSNR: {:.3f} - val SSIM: {:.3f}\n".format(e,
                                                                                                         ...
                                                                                                         (tot_val_psnr / (val_im_cntr+0.001)),
                                                                                                         (tot_val_ssim / (val_im_cntr+0.001))))
                save_path = saver.save(sess, self.ckpt_path + "edsr_ckpt")
    ...
    def upscale(self, path):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            cv2.imwrite("./images/EdsrOutput_" + tail, HR_image)
            cv2.imwrite("./images/BicubicOutput_" + tail, bicubic_image)
            cv2.imwrite("./images/original" + tail, fullimg)
