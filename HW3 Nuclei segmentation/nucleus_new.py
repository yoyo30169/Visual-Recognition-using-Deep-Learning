...
ROOT_DIR = os.path.abspath("/home/yoyo30169/VRDL_dataset/HW3_dataset/Mask_RCNN/")
...
class NucleusConfig(Config):
    ...
    STEPS_PER_EPOCH = 24 // IMAGES_PER_GPU
...
class NucleusDataset(utils.Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        subset_dir = "train" if subset in ["train"] else subset
    ...
    def build_coco_results(dataset, image_ids, rois, class_ids, scores, masks):
        ...
        for image_id in image_ids:
            ...
            for i in range(rois.shape[0]):
                ...
                # if(tmp):
                #     print(mask)
                #     tempseg =  binary_mask_to_rle(np.asfortranarray(mask))
                #     print(tempseg)
                #     tmp=False
                # tmpseg = mask_to_rle(class_id,mask,score)    
                tmpbbox =  [bbox[1], bbox[0], bbox[3] - bbox[1], bbox[2] - bbox[0]]
                
                result = {
                    "image_id": image_id+1,
                    "category_id":dataset.get_source_class_id(class_id, "nucleus"),
                    "bbox":np.array(tmpbbox).tolist() ,
